    ...
    no_obj=false, floor_area=false)
{
    ...
    tran_x_offset = (valve_r+fl_out_len);
    tran_y_offset = (valve_r+pn_out_len);
    scale([px, px, layer])
    translate([xpos, ypos, zpos])
    translate([(pitch-chan_w/2),(pitch-chan_w/2),offset_layers])
    translate([tran_x_offset, tran_y_offet, fl_out_h+chan_h])
    orient(2*tran_x_offset, 2*tran_y_offset, orientation)
            obj();
}
