...
module p_valve_4way(xpos, ypos, zpos, orientation,
    ...
    fl_extra_sp=10, pn_extra_sp=10, pn_up_layers=10, rot_pn=false,
     chan_h=10, chan_w=14, shape="cube", pitch=30, offset_layers=10,
    no_obj=false, floor_area=false)
{
    ...
    module obj()
    {
        ...
        //    (fl_extra_sp=="fill"?
        //        -(valve_r-chan_w/2-1):
        //        -((valve_r/4+fl_extra_sp))));
        inp_pos = -((valve_r/4+fl_extra_sp));
    ...
    tran_offset = (out_len+valve_r);
    translate([xpos, ypos, zpos])
    orient(pitch-chan_w, pitch-chan_w, orientation)
    translate([(pitch-chan_w/2),(pitch-chan_w/2),offset_layers])
    translate([tran_offset,tran_offset,(20+chan_h)])
        obj();
}
