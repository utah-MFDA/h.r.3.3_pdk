    ...
    no_obj=false, floor_area=false, report_len=false, chan_layers=2, clr="RosyBrown", layer_offset=20, alt=0, rot=0)
{
    ...
    x_off = L1/2+(pitch+chan_w/2);
    y_off = L2*(turns)/2+(pitch);

    translate([xpos, ypos, zpos])
    ///translate([L1/2+(pitch+chan_w/2), L2*(turns+1)/2+(pitch+chan_w/2), (chan_h/2+layer_offset)])
    orient(2*x_off, 2*y_off, orientation)
    translate(x_off, y_off, chan_h/2+layer_offset)
            obj();

    if(floor_area && floor_area != "transparent"){
        translate([xpos-chan_w/2, ypos-chan_w/2, zpos]){
        color("blue")
        translate([(pitch), (pitch),-layer/10])
            cube([(L1+chan_w), L2*(turns)+chan_w, layer/10]);
        color("red")
        translate([0, 0,-layer*2/10])
            cube([(L1+chan_w+pitch*2), L2*(turns)+(chan_w+pitch*2), layer/10]);
        }
    }
    if (report_len) {
        len_of_serp = (L1*(turns+1) + L2*turns)*chan_layers +
        (chan_layers-1)*lpv ; // via length
    ...
    if(floor_area == "transparent"){
        color("blue")
        translate([(pitch-chan_w/2), (pitch-chan_w/2),-layer/10])
            %cube([(L1+chan_w), L2*(turns)+chan_w, layer/10]);
        color("red")
        translate([0, 0,-layer*2/10])
            %cube([(L1+chan_w+pitch*2), L2*(turns)+(chan_w+pitch*2), layer/10]);
