...
class StraightChannel(object):
    ...
    def __init__(self, len, eta, node_in, node_out):
        super(straight_channel, self).__init__()
        self.len = len
        self.eta = eta
    ...
    def calcPressureOut(self):
        self.node_out.pressure = self.node_in.pressure - self.node_in.flow*self.r_hyd

    def calcPressureIn(self):
        self.node_in.pressure = self.node_out.pressure + self.node_in.flow*self.r_hyd

    def calcFlow(self):
        self.node_in.flow = (self.node_in.pressure-self.node_out.pressure)/self.r_hyd
        self.node_out.flow = (self.node_in.pressure-self.node_out.pressure)/self.r_hyd
