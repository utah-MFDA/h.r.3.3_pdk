...
sed "s/MAX_ROUTING_LAYER =.*/MAX_ROUTING_LAYER = met${metr}/" h.r.3.3/config.mk > h.r.3.3_met${met}/config.mk
sed -i "s/ZBULK_VAL.*=.*/ZBULK_VAL = ${zbulk}/" h.r.3.3_met${met}/config.mk
echo "export FLOW_VARIANT = met${met}" >> h.r.3.3_met${met}/config.mk
