...
module flush_hole_0(xpos, ypos, zpos, orient){
    translate([xpos*px, ypos*px, (zpos+180)*layer])
    orientation(80*px, 20*px, orient)
    cube([80*px, 20*px, 20*px]);
}
...
module pinhole_320px_0(xpos, ypos, zpos, orient, d=140, l=260, h=67){
    ...
    translate([xpos*px, ypos*px, (zpos+45)*layer])
    orientation(l_pin+h_cone, d_pin, orient)
    obj();

}

module interconnect_8x4(xpos, ypos, zpos, orient, width=20, height=20, depth=20, size=20, columns = 8, rows=4, spacing=90) {
    x_dimm = width*px;
    y_dimm = height*px;
    z_dimm = depth*layer;
    grid_offset = spacing*px;
    oaw = (columns-1)*grid_offset+x_dimm;
    oah = (rows-1)*grid_offset+y_dimm;
    translate([xpos*px, ypos*px, (zpos+360)*layer])
    orientation(oaw, oah, orient)
    union() {
        for (row=[0:rows-1]){
            for (col=[0:columns-1]) {
                translate([grid_offset*col, grid_offset*row, 0])
                cube([x_dimm, y_dimm, z_dimm]);
            }
        };
        %cube([oaw, oah, z_dimm]);
    }
...
module interconnect_8x4_postitive(xpos, ypos, zpos, orient) {
    gasket_side_length = 40*px;
    gasket_aperture_length = 20*px;
    gasket_height = 1*layer;
    ...
    translate([xpos*px, ypos*px, (zpos+260)*layer])
    // Overall length
